          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Badge variant="destructive" className="mb-2 capitalize">
                {caseData.urgency.level} - {caseData.urgency.score}/10
              </Badge>
              <p className="text-sm text-red-800">{caseData.urgency.summary}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-gray-700 leading-relaxed">
              The AI analysis has identified {caseData.findings.length} areas of concern in your dental X-ray. 
              {caseData.urgency.level === 'urgent' && ' Some of these require prompt attention to prevent further complications.'}
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <ul className="space-y-1 text-sm text-yellow-800">
                {caseData.findings
                  .filter((f: any) => f.urgency === 'attention')
                  .map((f: any) => (
                    <li key={f.id}>• {f.condition} - Tooth {f.tooth}</li>
                  ))}
                {caseData.findings.filter((f: any) => f.urgency === 'attention').length === 0 && (
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <ul className="space-y-1 text-sm text-green-800">
                {caseData.findings
                  .filter((f: any) => f.urgency === 'routine')
                  .map((f: any) => (
                    <li key={f.id}>• {f.condition} - Tooth {f.tooth}</li>
                  ))}
              </ul>
          ...
          <CardContent className="p-4">
            <ol className="space-y-2 text-sm text-gray-700">
              <li className="flex gap-2">
                ...
                <span>{caseData.urgency.recommendation}</span>
              </li>
              <li className="flex gap-2">

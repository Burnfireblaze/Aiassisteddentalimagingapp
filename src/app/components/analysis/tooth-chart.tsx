...
export default function ToothChart({ findings, onToothSelect, selectedTooth }: ToothChartProps) {
  ...
  const getToothColor = (urgency: string | undefined) => {
    if (!urgency) return 'bg-gray-100 hover:bg-gray-200';
    switch (urgency) {
      case 'urgent':
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div className="space-y-1 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{getToothStatus(selectedTooth)?.condition}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-gray-600">Urgency:</span>
                  <Badge 
                    variant={getToothStatus(selectedTooth)?.urgency === 'urgent' ? 'destructive' : 'default'}
                    className="capitalize"
                  >
                    {getToothStatus(selectedTooth)?.urgency}
                  </Badge>
                </div>
              </div>
            </div>
